    ...
    for (Long64_t i = 0; i < 1000; ++i) {
        ...
        for (int k =0 ; k < nref; ++k) {
            ...
            if (!Nodes.empty()) {
                ...
                Node *SDNode = nullptr;  // donesnt point to anything yet. We are replacing FindSDnode fuction with logic
                {
                    // initiazling parameters for the soft drop conditions, these are adjustable if needed
                    ...
                    //now we write down the same exact chunck from FindSDnode
                    if(HeadNode != NULL){
                    ...
                    if (SDNode == nullptr) {
                        std::cerr << "FindSDNode failed to find a valid node." << std::endl;
                    }
                }
